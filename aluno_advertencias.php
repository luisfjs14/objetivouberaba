...
<div class="container">
  ...
  <h2 class="titulo-secao">Advertências de <?=htmlspecialchars($aluno["nome"])?></h2>
  <?php if (empty($advertencias)): ?>
    <p>Este aluno não possui nenhuma advertência registrada.</p>
  <?php else: ?>
    <?php foreach ($advertencias as $a): ?>
      <div class="card">
        <div class="titulo"><?=htmlspecialchars($a["titulo"])?></div>
        <div><?=nl2br(htmlspecialchars($a["motivo"]))?></div>
        <small>
          Data: <?=date("d/m/Y", strtotime($a["data_registro"]))?><br>
          Registrado por: <?=htmlspecialchars($a["funcionario"])?><br>
          <a href="documentos/advertencias/<?=htmlspecialchars($a["arquivo_pdf"])?>" target="_blank" class="btn-pdf">Visualizar PDF</a>
        </small>
      </div>
    <?php endforeach; ?>
  <?php endif; ?>
